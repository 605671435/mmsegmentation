...
class SK_Module(nn.Module):
    def __init__(self,
        ...
        # self.conv_down = nn.Conv2d(in_channels, channels, kernel_size=1, bias=bias)
        # self.conv_up = nn.Conv2d(channels, in_channels, kernel_size=1, bias=bias)
        self.bottleneck = nn.Sequential(
    ...
    # def reset_parameters(self):
        ...
        # kaiming_init(self.conv_down, mode='fan_in')
        # kaiming_init(self.conv_up, mode='fan_in')
        # self.conv_down.inited = True
...
class EX_Module_legacy(nn.Module):
    def __init__(self,
        ...
        self.sk = SK_Module(in_channels=in_channels, conv_cfg=conv_cfg)
...
class EX_Module(nn.Module):
    def __init__(self,
        ...
        if fusion_type == 'dsa':
            ...
            # self.sk_conv_down = nn.Conv2d(in_channels=in_channels, out_channels=in_channels // 2, kernel_size=1, bias=False)
            # self.sk_conv_up = nn.Conv2d(in_channels=in_channels // 2, out_channels=in_channels, kernel_size=1, bias=False)
            # self.sk_softmax = nn.Softmax(1)
    ...
    # def sk(self, x1, x2):
    #     u = self.sk_gap(x1 + x2)
    #     u = self.sk_conv_down(u)
    #     u = self.sk_conv_up(u)
    #     softmax_a = self.sk_softmax(u)
    #     out = x1 * softmax_a + x2 * (1 - softmax_a)
    #     return out
